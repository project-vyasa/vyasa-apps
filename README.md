...
Nested dests (`viewer`, `sanskrit`, `docs`, later `studio`) are first path segments under the project site.

Shared Starlight chrome: [`vyasa-ui/starlight`](../vyasa-ui/starlight/README.md) (`@project-vyasa/starlight-theme`).

```bash
bun run assemble:pages   # build viewer + studio + docs → dist-pages/
