...
describe('catalog-ref', () => {
	...
	it('builds reader and explore paths', () => {
		expect(publicationReaderPath(ref, '2:47', '/vyasa-apps/viewer')).toBe(
			'/vyasa-apps/viewer/adi/vysamples/vyasa-bg/2:47'
		);
		expect(publicationExplorePath(ref, '')).toBe('/adi/vysamples/vyasa-bg/explore');
		expect(registryLibraryPath('adi', '/vyasa-apps/viewer')).toBe('/vyasa-apps/viewer/adi');
	...
	it('serializes vyasa URI and HTTP link', () => {
		...
		expect(catalogLinkToHttp(link, 'https://project-vyasa.github.io', '/vyasa-apps/viewer')).toBe(
			'https://project-vyasa.github.io/vyasa-apps/viewer/adi/vysamples/vyasa-bg/2:47'
		);
	});
	...
	it('reserves sibling app path segments', () => {
		...
		expect(RESERVED_REGISTRY_IDS.has('docs')).toBe(true);
